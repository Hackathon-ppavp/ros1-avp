...
void MPCFollower::callbackRefPath(const autoware_msgs::Lane::ConstPtr &msg)
{
  ...
  if (enable_path_smoothing_)
  {
    for (int i = 0; i < path_smoothing_times_; ++i)
    {
      ...
      {
        // TODO::Change to debug- there is a problem at the last waypoint which has one or none
        //       This will bring up the warning!
        DEBUG_INFO("[MPC] path callback: filtering error. stop filtering");
        return;
      }
    }
  }
  ...
  if (!traj.size())
  {
    // TODO:: Change ROS ERROR to DEBUG Level. Somewhere close to the final point has not enough waypoints for computing trajectory
