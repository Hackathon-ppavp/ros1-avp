...
private:
  ...
  {
    ROS_INFO("final waypoint received");
    ...
    for (int i = 0; i < (int)base_waypoints_.waypoints.size(); ++i) {
      ...
      // TODO:: Problematic when the vehicle doesn't quite reach the goal point within this region
      //        it will print ROS_ERROR("cannot find closest base_waypoints ...
      //        allow more threshold or remove the following line and move it to other node to handle it e.g. waypoint tracker
      if (d < 0.01) {
        closest_idx = i;
        break;
      }
    ...
    if (closest_idx == -1) {
      // TODO:: Change it to Debug level: See the reason above
      ROS_INFO("cannot find closest base_waypoints' waypoint to final_waypoints.waypoint[1] !!");
